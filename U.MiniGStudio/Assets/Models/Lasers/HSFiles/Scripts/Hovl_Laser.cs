...
{
    [SerializeField] private Transform _rouroule;
    ...
    void Update()
    {
        ...
        {
            Laser.SetPosition(0, _laserPoint.position);
            ...
            Vector3 dir = (_hitPoint.position - _laserPoint.position).normalized;
            _rouroule.rotation = Quaternion.LookRotation(dir, Vector3.up);
            if (Physics.Raycast(_laserPoint.position, dir, out hit, MaxLength))//CHANGE THIS IF YOU WANT TO USE LASERRS IN 2D: if (hit.collider != null)
            {
            ...
            else
            {
                ...
                var EndPos = _laserPoint.position + dir * MaxLength;
